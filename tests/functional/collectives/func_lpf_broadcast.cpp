...
void spmd( lpf_t ctx, lpf_pid_t s, lpf_pid_t p, lpf_args_t args )
{
    ...
    rc = lpf_collectives_init( ctx, s, p, 0, NULL, 1, 0, (1<<19), &coll );
    EXPECT_EQ( LPF_SUCCESS, rc );
...
/** 
 * \test Initialises one \a lpf_coll_t object, performs a broadcast, and deletes it.
 * \pre P >= 1
 * \return Exit code: 0
 */
TEST( COLL, func_lpf_broadcast )
{
