...
void spmd( lpf_t ctx, lpf_pid_t s, lpf_pid_t p, lpf_args_t args)
{
    ...
    rc = lpf_collectives_init( ctx, s, p, 0, NULL, 1, 0, 0, &coll1 );
    EXPECT_EQ( LPF_SUCCESS, rc );

    rc = lpf_collectives_init( ctx, s, p, 0, NULL, 1, 8, 0, &coll2 );
    EXPECT_EQ( LPF_SUCCESS, rc );

    rc = lpf_collectives_init( ctx, s, p, 0, NULL, (1<<7), 0, (1<<19), &coll3 );
    EXPECT_EQ( LPF_SUCCESS, rc );

    rc = lpf_collectives_init( ctx, s, p, 0, NULL, (1<<4), 8, (1<<25), &coll4 );
    EXPECT_EQ( LPF_SUCCESS, rc );
 ...
 * \return Exit code: 0
 */
TEST( COLL, func_lpf_collectives_init )
{
