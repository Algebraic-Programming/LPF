...
void spmd(lpf_t ctx, lpf_pid_t s, lpf_pid_t p, lpf_args_t args) {
  ...
  // overflow in the number of calls: may or may not be encountered by an
  // implementation:
  const lpf_err_t rc1 =
      lpf_collectives_init(ctx, s, p, tooBig, (1 << 7), (1 << 7), &coll2);
  bool success = (rc1 == LPF_SUCCESS) || (rc1 == LPF_ERR_OUT_OF_MEMORY);
  EXPECT_EQ(true, success);

  // overflow in the element size required for reduction buffers: an
  // implementation MUST detect this:
  rc = lpf_collectives_init(ctx, s, p, (1 << 7), tooBig, (1 << 7), &coll3);
  EXPECT_EQ(LPF_ERR_OUT_OF_MEMORY, rc);

  // overflow in the collective buffer size: may or may not be encountered by an
  // implementation:
  const lpf_err_t rc2 =
      lpf_collectives_init(ctx, s, p, (1 << 7), (1 << 7), tooBig, &coll4);
  success = (rc2 == LPF_SUCCESS) || (rc2 == LPF_ERR_OUT_OF_MEMORY);
  EXPECT_EQ(true, success);

  // overflow that if not detected would lead to a very small buffer: an
  // implementation MUST detect this:
  if (p > 1) {
    rc = lpf_collectives_init(ctx, s, p, (1 << 7), tooBig / p + 1, (1 << 7),
                              &coll5);
    EXPECT_EQ(LPF_ERR_OUT_OF_MEMORY, rc);
  }
...
/**
 * \test Checks four ways in which a buffer could overflow; two of them MUST be
 * detected-- for the other two ways this test accepts an LPF_ERR_OUT_OF_MEMORY
 * and accepts a LPF_SUCCESS. \pre P >= 1 \return Exit code: 0
 */
