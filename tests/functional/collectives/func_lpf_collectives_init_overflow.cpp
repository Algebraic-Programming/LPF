...
void spmd( lpf_t ctx, lpf_pid_t s, lpf_pid_t p, lpf_args_t args)
{
    ...
    rc = lpf_collectives_init( ctx, s, p, 0, NULL, (1<<7), (1<<7), (1<<7), &coll1 );
    EXPECT_EQ( LPF_SUCCESS, rc );
    ...
    const lpf_err_t rc1 = lpf_collectives_init( ctx, s, p, 0, NULL, tooBig, (1<<7), (1<<7), &coll2 );
    ...
    rc = lpf_collectives_init( ctx, s, p, 0, NULL, (1<<7), tooBig, (1<<7), &coll3 );
    ...
    const lpf_err_t rc2 = lpf_collectives_init( ctx, s, p, 0, NULL, (1<<7), (1<<7), tooBig, &coll4 );
    ...
    if( p > 1 ) {
        rc = lpf_collectives_init( ctx, s, p, 0, NULL, (1<<7), tooBig / p + 1, (1<<7), &coll5 );
        EXPECT_EQ( LPF_ERR_OUT_OF_MEMORY, rc );
    }
