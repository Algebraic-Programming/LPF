...
void spmd(lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args) {
  ...
  rc = lpf_sync(lpf, LPF_SYNC_DEFAULT);
  EXPECT_EQ(LPF_SUCCESS, rc);
  rc = lpf_put(lpf, aSlot, 0, (pid + 1) % nprocs, aSlot, sizeof(a[0]),
               sizeof(a[0]), LPF_MSG_DEFAULT);
  EXPECT_EQ(LPF_SUCCESS, rc);
  rc = lpf_sync(lpf, LPF_SYNC_DEFAULT);
  EXPECT_EQ(LPF_SUCCESS, rc);
...
/**
 * \test Test single lpf_exec() call with a single arg on all processors. All
 * processes other than process 1 perform an extra sync. \pre P >= 2 \return
 * Exit code: 0
 */
