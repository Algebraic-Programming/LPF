...
void spmd(lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args) {
  ...
  EXPECT_EQ((size_t)(bsplib_pid(bsplib) == 0 ? bsplib_nprocs(bsplib) : 0),
            nmsg);
  EXPECT_EQ(
      (size_t)(bsplib_pid(bsplib) == 0 ? bsplib_nprocs(bsplib) * sizeof(x) : 0),
      bytes);
  ...
  EXPECT_EQ(bsplib_pid(bsplib) == 0 ? bsplib_nprocs(bsplib) : 0,
            (unsigned)nMessages);
  ...
  EXPECT_EQ(bsplib_pid(bsplib) == 1 ? bsplib_nprocs(bsplib) : 0,
            (unsigned)nMessages);
