...
void spmd(lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args) {
  ...
  if (pid % 2 == 1) {
    rc = lpf_get(lpf, 0, xSlot, sizeof(int), ySlot, sizeof(int), sizeof(int),
                 LPF_MSG_DEFAULT);
    EXPECT_EQ(LPF_SUCCESS, rc);
  }
  rc = lpf_sync(lpf, LPF_SYNC_DEFAULT);
...
/**
 * \test Testing for a program with a lpf_put and a lpf_get to a remote process
 * which can only handle 2 requests \pre P >= 3 \return Message: Too many
 * messages on pid 0. Reserved was 2 while there were actually \return Exit
 * code: 6
 */
TEST(API, func_lpf_debug_put_get_too_many_requests_remote) {
