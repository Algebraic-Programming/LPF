...
void spmd(lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args) {
  ...
  rc = lpf_get(lpf, 6, xSlot, 0, ySlot, 0, sizeof(int), LPF_MSG_DEFAULT);
  FAIL();
}

/**
 * \test Testing for a process that does a lpf_get() from another process that
 * does not exist \pre P >= 1 \pre P <= 5 \return Message: unknown process ID 6
 * for data source \return Exit code: 6
 */
TEST(API, func_lpf_debug_get_unknown_source_pid) {
