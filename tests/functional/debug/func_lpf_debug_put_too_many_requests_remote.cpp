...
void spmd(lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args) {
  ...
  if (pid % 2 == 0) {
    rc = lpf_put(lpf, xSlot, 0, (pid + 1) % 2, ySlot, 0, sizeof(int),
                 LPF_MSG_DEFAULT);
    EXPECT_EQ(LPF_SUCCESS, rc);
  }

  if (pid % 2 == 1) {
    rc = lpf_put(lpf, xSlot, sizeof(int), pid % 2, ySlot, sizeof(int),
 ...
 * \pre P >= 2
 * \return Message: Too many messages on pid 1. Reserved was 1 while there were
 * actually \return Exit code: 6
 */
