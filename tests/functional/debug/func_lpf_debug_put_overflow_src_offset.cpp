...
void spmd( lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args )
{
    ...
    rc = lpf_put( lpf, xSlot, 2, (pid+1)%nprocs, ySlot, 0, -1, LPF_MSG_DEFAULT );
    EXPECT_EQ( LPF_SUCCESS, rc );
    FAIL();
...
/** 
 * \test Source offset + size in lpf_put overflows
 * \pre P >= 1
 * \return Message: numerical overflow while computing src_offset
 * \return Exit code: 6
 */
TEST( API, func_lpf_debug_put_overflow_src_offset )
{
