...
void spmd( lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args )
{
    ...
    EXPECT_DEATH(lpf_get( lpf, (pid+1)%nprocs, xSlot, 0, ySlot, 2, -1, LPF_MSG_DEFAULT ), "numerical overflow while");

}

/** 
 * \test Destination offset + size in lpf_get overflows
 * \pre P >= 1
 * \return Message: numerical overflow while computing dst_offset
 * \return Exit code: 6
 */
TEST( API, func_lpf_debug_get_overflow_dst_offset )
{
