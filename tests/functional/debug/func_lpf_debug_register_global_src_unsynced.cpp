...
void spmd(lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args) {
  ...
  rc = lpf_get(lpf, (pid + 1) % nprocs, xSlot, 0, ySlot, 0, sizeof(x),
               LPF_MSG_DEFAULT);
  EXPECT_EQ(LPF_SUCCESS, rc);
  FAIL();
}
...
/**
 * \test Testing a lpf_put with an inactive source memory slot
 * \pre P >= 1
 * \return Message: source memory slot was not yet active
 * \return Exit code: 6
 */
TEST(API, func_lpf_debug_register_global_src_unsynced) {
