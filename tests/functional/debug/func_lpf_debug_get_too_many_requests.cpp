...
void spmd( lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args )
{
    ...
    rc = lpf_get( lpf, (pid+1)%nprocs, xSlot, 0, ySlot, 0, sizeof(int), LPF_MSG_DEFAULT );
    EXPECT_EQ( LPF_SUCCESS, rc );

    EXPECT_DEATH(lpf_get( lpf, (pid+2)%nprocs, xSlot, sizeof(int), ySlot, sizeof(int), sizeof(int), LPF_MSG_DEFAULT ), "LOL");
...
/** 
 * \test Testing for a process that issues more lpf_get requests than allocated with lpf_resize_message_queue()
 * \pre P >= 1
