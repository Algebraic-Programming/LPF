...
void spmd( lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args )
{
    ...
    EXPECT_DEATH(lpf_deregister( lpf, pid == 0 ? xSlot : ySlot ), "LOL");
...
/** 
 * \test A program with a lpf_deregister that does not match the same slot
 * \pre P >= 2
 * \return Message: global deregistration mismatches
 * \return Exit code: 6
 */
TEST( API, func_lpf_debug_global_deregister_mismatch )
{
