...
void spmd(lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args) {
  ...
  rc =
      lpf_put(lpf, xSlot, 0, (pid + 1) % nprocs, ySlot, 2, -1, LPF_MSG_DEFAULT);
  EXPECT_EQ(LPF_SUCCESS, rc);
...
/**
 * \test Destination offset + size in lpf_put overflows
 * \pre P >= 1
 * \return Message: numerical overflow while computing dst_offset
 * \return Exit code: 6
 */
TEST(API, func_lpf_debug_put_overflow_dst_offset) {
