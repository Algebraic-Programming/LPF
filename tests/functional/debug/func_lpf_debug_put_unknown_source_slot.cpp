...
void spmd(lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args) {
  ...
  rc = lpf_put(lpf, ySlot, 0, (pid + 1) % nprocs, xSlot, 0, sizeof(x),
               LPF_MSG_DEFAULT);
  EXPECT_EQ(LPF_SUCCESS, rc);
...
/**
 * \test Testing a lpf_put with an unknown source memory slot
 * \pre P >= 1
 * \return Message: source memory slot does not exist
 * \return Exit code: 6
 */
TEST(API, func_lpf_debug_put_unknown_source_slot) {
