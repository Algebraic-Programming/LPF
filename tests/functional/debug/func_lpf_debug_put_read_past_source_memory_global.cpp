...
void spmd(lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args) {
  ...
  rc = lpf_put(lpf, xSlot, 1, (pid + 1) % nprocs, ySlot, 0, sizeof(x),
               LPF_MSG_DEFAULT);
  EXPECT_EQ(LPF_SUCCESS, rc);
  FAIL();
}
...
/**
 * \test Testing for a lpf_put() that reads past globally registered memory
 * bounds \pre P >= 1 \return Message: source memory .* is read past the end by
 * 1 bytes \return Exit code: 6
 */
TEST(API, func_lpf_debug_put_read_past_source_memory_global) {
