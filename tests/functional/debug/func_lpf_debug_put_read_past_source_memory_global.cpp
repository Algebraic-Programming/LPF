...
void spmd( lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args )
{
    ...
    rc = lpf_put( lpf, xSlot, 1, (pid+1)%nprocs, ySlot, 0, sizeof(x), LPF_MSG_DEFAULT );
    EXPECT_EQ( LPF_SUCCESS, rc );

    EXPECT_DEATH(lpf_sync( lpf, LPF_SYNC_DEFAULT ), ":");

    EXPECT_EQ( 3, y );
}

/** 
 * \test Testing for a lpf_put() that reads past globally registered memory bounds
 * \pre P >= 1
