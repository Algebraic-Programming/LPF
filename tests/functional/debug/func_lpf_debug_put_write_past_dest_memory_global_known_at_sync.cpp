...
void spmd( lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args )
{
    ...
    rc = lpf_put( lpf, xSlot, 0, (pid+1)%nprocs, ySlot, 3, sizeof(x), LPF_MSG_DEFAULT );
    EXPECT_EQ( LPF_SUCCESS, rc );
...
/** 
 * \test Testing for a lpf_put() that writes past globally registered memory bounds
 * \pre P >= 1
 * \return Message: destination memory .* is written past the end by 3 bytes
 * \return Exit code: 6
 */
TEST( API, func_lpf_debug_put_write_past_dest_memory_global_known_at_sync )
{
