...
void spmd( lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args )
{
    ...
    rc = lpf_get( lpf, (pid+1)%nprocs, xSlot, 0, ySlot, 2, sizeof(x), LPF_MSG_DEFAULT );
    EXPECT_EQ("%d", LPF_SUCCESS, rc );
...
/** 
 * \test Testing for a lpf_get() that writes past locally registered memory bounds
 * \pre P >= 1
 * \return Message: destination memory .* is written past the end by 2 bytes
 * \return Exit code: 6
 */
TEST( func_lpf_debug_get_write_past_dest_memory_local )
{
