...
void spmd(lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args) {
  ...
  for (i = 1, j = 0; i <= m; j += i, ++i) {
    rc = bsplib_hpput(bsplib, (bsplib_pid(bsplib) + 1) % bsplib_nprocs(bsplib),
                      value, array, j * sizeof(uint32_t), i * sizeof(uint32_t));
    EXPECT_EQ(BSPLIB_SUCCESS, rc);
  }
...
/**
 * \test Tests sending a lot of messages through bsp_hpput
 * \pre P >= 1
 * \return Exit code: 0
 */
