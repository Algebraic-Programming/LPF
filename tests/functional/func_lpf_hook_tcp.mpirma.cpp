...
void spmd(lpf_t ctx, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args) {
  ...
  rc = lpf_put(ctx, xSlot, 0, (pid + 1) % nprocs, ySlot, 0, sizeof(x),
               LPF_MSG_DEFAULT);
  EXPECT_EQ("%d", rc, LPF_SUCCESS);
...
/**
 * \test Tests lpf_hook on mpi implementation using TCP/IP to initialize. The
 * pids and nprocs are checked for their correctness. \pre P >= 1 \return Exit
