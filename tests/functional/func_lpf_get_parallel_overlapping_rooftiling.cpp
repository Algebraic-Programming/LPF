...
void spmd(lpf_t lpf, lpf_pid_t pid, lpf_pid_t nprocs, lpf_args_t args) {
  ...
  if (0 == pid) {
    for (i = 0; i < nprocs; ++i) {
      ...
      rc = lpf_get(lpf, i, xslot, start * sizeof(xs[0]) * MTU, yslot,
                   offset * sizeof(xs[0]) * MTU, length * sizeof(xs[0]) * MTU,
                   LPF_MSG_DEFAULT);
      EXPECT_EQ(LPF_SUCCESS, rc);
    }
  }
  ...
  if (0 == pid) {
    ...
    for (i = 0; i < nprocs; ++i) {
      ...
      for (j = 0; j < 5; ++j) {
        uint64_t fromPid =
            ys[(4 * i + j) * MTU] - xs[(4 * i + j + offset) * MTU];
        fromPid = (fromPid + nprocs * MTU) % nprocs;
        for (k = 0; k < MTU; ++k) {
          uint64_t fromPid2 =
              ys[(4 * i + j) * MTU + k] - xs[(4 * i + j + offset) * MTU + k];
          fromPid2 = (fromPid2 + nprocs * MTU) % nprocs;
          EXPECT_EQ(fromPid, fromPid2);

          if (fromPid == i) {
            EXPECT_EQ((5 * i + j) * nprocs * MTU + fromPid,
                      ys[(4 * i + j) * MTU]);
          }
        }
...
/**
 * \test Test lpf_get writing to partial overlapping memory areas from multiple
 * processors. \pre P >= 1 \return Exit code: 0
 */
TEST(API, func_lpf_get_parallel_overlapping_rooftiling) {
