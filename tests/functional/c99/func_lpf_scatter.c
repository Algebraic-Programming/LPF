...
void spmd( lpf_t ctx, const lpf_pid_t s, lpf_pid_t p, lpf_args_t args )
{
    ...
    if( s == p / 2 ) {
        for( size_t i = 0; i < size * p; ++i ) {
            EXPECT_EQ( "%c", (char)i, data[i] );
        }
    } else {
        for( size_t i = 0; i < size; ++i ) {
            EXPECT_EQ( "%c", (char)(s * size + i), data[i] );
        }
    }
...
/** 
 * \test Initialises one \a lpf_coll_t object, performs a scatter, and deletes the \a lpf_coll_t object.
 * \pre P >= 1
 * \return Exit code: 0
 */
TEST( func_lpf_scatter )
{
